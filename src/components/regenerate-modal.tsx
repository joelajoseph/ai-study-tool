  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 p-4 backdrop-blur-xs">
      <div
        ...
        className="relative max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-3xl bg-white p-6 shadow-xl ring-1 ring-slate-200 sm:p-8"
      >
        <div className="flex items-start justify-between gap-4 border-b border-slate-100 pb-4">
          <div>
            <span className="inline-block text-xs font-semibold uppercase tracking-[0.16em] text-emerald-700">
              Mid-way Adjustment
            </span>
            <h2 id="modal-title" className="mt-1 text-2xl font-semibold text-slate-950">
        ...
        {preview ? (
          <div className="mt-5 space-y-5">
            <div className="rounded-2xl bg-emerald-50/70 p-4 ring-1 ring-emerald-100">
              <h3 className="text-sm font-semibold text-emerald-900">Proposed Schedule Preview</h3>
              <p className="mt-1 text-sm text-emerald-800">{preview.plan.overview}</p>
            ...
            <div className="space-y-3">
              ...
              <ol className="space-y-2.5 max-h-60 overflow-y-auto pr-1">
                {preview.plan.topics.map((topic, i) => (
                  <li key={i} className="rounded-xl border border-slate-200 bg-slate-50/70 p-3 text-sm">
                    <div className="flex items-center justify-between gap-2">
                      <div className="flex items-center gap-2">
                        <span className="flex h-5 w-5 items-center justify-center rounded-full bg-emerald-100 text-xs font-bold text-emerald-800">
                          {i + 1}
                        </span>
                    ...
                    <p className="mt-1.5 text-xs text-slate-600">{topic.rationale}</p>
                  </li>
                ))}
              </ol>
            </div>
            ...
            <div className="flex flex-wrap items-center justify-end gap-3 pt-3 border-t border-slate-100">
              ...
              <button
                ...
                onClick={handleApply}
                disabled={isApplying}
                className="rounded-xl bg-emerald-700 px-4 py-2.5 text-sm font-semibold text-white hover:bg-emerald-800 transition disabled:opacity-50"
              >
          ...
          <form onSubmit={handleGeneratePreview} className="mt-5 space-y-4">
              ...
              {completedTopics.length > 0 && (
                <div className="mt-2.5 flex flex-wrap gap-1.5">
                  {completedTopics.map((topic) => (
                    <span
                      key={topic.id}
                      className="inline-flex items-center gap-1 rounded-full bg-white px-2.5 py-0.5 text-xs font-medium text-slate-700 ring-1 ring-slate-200"
                    >
                      <span className="text-emerald-600 font-bold">✓</span> {topic.title}
                    </span>
                  ))}
                </div>
              )}
            ...
            <div>
              <label htmlFor="regen-exam-date" className="block text-sm font-semibold text-slate-900">
                Assessment date
              </label>
              <div className="mt-1.5 flex items-center gap-3">
                <input
                  ...
                  className="w-full rounded-xl border border-slate-300 bg-white px-3.5 py-2.5 text-sm outline-none transition focus:border-emerald-600 focus:ring-4 focus:ring-emerald-100"
                  required
                />
                <span className="shrink-0 text-xs font-medium text-slate-600 bg-slate-100 px-3 py-2 rounded-xl">
            ...
            <div>
              <label htmlFor="regen-notes" className="block text-sm font-semibold text-slate-900">
                What feels shaky or has changed? <span className="font-normal text-slate-500">(optional)</span>
              </label>
              <textarea
                id="regen-notes"
                rows={3}
                ...
                className="mt-1.5 w-full resize-y rounded-xl border border-slate-300 bg-white px-3.5 py-2.5 text-sm leading-6 outline-none transition placeholder:text-slate-400 focus:border-emerald-600 focus:ring-4 focus:ring-emerald-100"
              />
            </div>
